...
void parallel_qsort_sort (int *T, const int size)
{
  if (size == 2) { 
    ...
    register int csize = size/nbthreads;
    register int i;

    #pragma omp parallel for schedule(runtime) shared(csize, T) private(i)
    for (i=0; i<nbthreads; i++){
        qsort(&(T[i*csize]), csize, sizeof(int), compare);
    }
    register int nsize = csize;
    while (nsize<size) {
        for (i=0; i<size/nsize/2; i++) {
          merge(&T[i * nsize * 2], nsize);
        }
        nsize = nsize*2;
      }
}
