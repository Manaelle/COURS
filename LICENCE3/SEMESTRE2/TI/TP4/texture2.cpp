...
void cylindre()
{
	...
	//EXO8
	Texture texture_BUS = Texture("BUS.ppm"); 
	Texture texture_UGA = Texture("UGA.ppm"); 
	
	glEnable(GL_TEXTURE_2D);
	
	// tracé de la base inférieure : cercle approchée par un polygone 
	//glColor3d(1.0,0.0,1.0); // magenta
	glBindTexture(GL_TEXTURE_2D, texture_BUS.numero());
	
	glBegin(GL_POLYGON);
	for (i=0; i<=N; i++)
	{
		double a = (2.0*M_PI*i)/N;
		double x = cos(a)*0.5+0.5;
		double y = sin(a)*0.5+0.5;
		glTexCoord2d(x,y);
	...
	glBindTexture(GL_TEXTURE_2D, texture_UGA.numero());
