...
void partie1()
{
	...
	ImageCouleurF I_HSV1(301,301);
	
	for (UINT x=0; x<=300; x++)
	for (UINT y=0; y<=300; y++)
	{
		I_HSV1.comp(x,y,0) = 2.0/3.0;        // composante H
		I_HSV1.comp(x,y,1) = (float)x/300.0; // composante S
		I_HSV1.comp(x,y,2) = (float)y/300.0; // composante V
	}
	
	// convertir l'image I_HSV du format HSV au format RGB 
	// --> image I_RGB
	ImageCouleurF I_RGB1;
	I_RGB1 = hsv2rgb(I_HSV1);
	...
	ImageCouleurF I_HSV2(301,301);
	
	for (UINT x=0; x<=300; x++)
	for (UINT y=0; y<=300; y++)
	{
		I_HSV2.comp(x,y,0) = (float)x/300.0; // composante H
		I_HSV2.comp(x,y,1) = 1;              // composante S
		I_HSV2.comp(x,y,2) = (float)y/300.0; // composante V
	}
	
	// convertir l'image I_HSV du format HSV au format RGB 
	// --> image I_RGB
	ImageCouleurF I_RGB2;
	I_RGB2 = hsv2rgb(I_HSV2);
	...
	ImageCouleurF I_HSV3(301,301);
	
	for (UINT x=0; x<=300; x++)
	for (UINT y=0; y<=300; y++)
	{
		I_HSV3.comp(x,y,0) = (float)x/300.0; // composante H
		I_HSV3.comp(x,y,1) = (float)y/300.0; // composante S
		I_HSV3.comp(x,y,2) = 1;              // composante V
	}
	
	// convertir l'image I_HSV du format HSV au format RGB 
	// --> image I_RGB
	ImageCouleurF I_RGB3;
	I_RGB3 = hsv2rgb(I_HSV3);
...
void partie2()
{
	...
	ImageCouleurF I_HSV1;
	I_HSV1 = rgb2hsv(I_RGB);
	
	//-------------------------------------------------
	// mise à 1.0 des 2ème et 3ème composantes (S et I) 
	for (UINT x=0; x<I_HSV1.largeur(); x++)
	for (UINT y=0; y<I_HSV1.hauteur(); y++)
	{
		I_HSV1.comp(x,y,1) = 1.0;
		I_HSV1.comp(x,y,2) = (float)x/(float)I_HSV.largeur();
	}
	
	// passage au mode RGB
	ImageCouleurF I2_RGB1;
	I2_RGB1 = hsv2rgb(I_HSV1);
	I2_RGB1.afficher();
	
	ImageCouleurF I_HSV2;
	I_HSV2 = rgb2hsv(I_RGB);
	
	//-------------------------------------------------
	// mise à 1.0 des 2ème et 3ème composantes (S et I) 
	for (UINT x=0; x<I_HSV2.largeur(); x++)
	for (UINT y=0; y<I_HSV2.hauteur(); y++)
	{
		I_HSV2.comp(x,y,1) = (float)x/(float)I_HSV.largeur();
		I_HSV2.comp(x,y,2) = 1.0;
	}
	
	// passage au mode RGB
	ImageCouleurF I2_RGB2;
	I2_RGB2 = hsv2rgb(I_HSV2);
	I2_RGB2.afficher();
