...
int main()
{
	...
	ImageGrisF I3(401,401);
	
	// définir les valeurs pixels
	for (int x=0; x<401; x++)
	for (int y=0; y<401; y++)
		I3.pixel(x,y) = ((float)x+(float)y) / 800.0;
	...
	for (int x=0; x<401; x++)
	for (int y=0; y<401; y++)
		I4.pixel(x,y) = ((float)x+(float)y) / 400.0 - 0.5;
	
	// affichage de l'image I2 avec un facteur d'échelle 20
	UINT num_I4 = I4.afficher(1);
	
	I3.sauvegarder("image3 exo6.png");
	I4.sauvegarder("image4 exo6.png");
