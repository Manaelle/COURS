...
#include "readelf_fusion_section.h"
#include "debug.h"
...
int main(int argc, char *argv[]) {
  ...
  if (option1) {
		print_header(donnees.ehdr);
  }

  if (option2) {
		print_sectionTable(donnees.ehdr, donnees.shdrTable, donnees.tabNames);
  }
  ...
  if (option3 != NULL) {
  	...
  	if(*error == 0){
       	print_contenuSection(fd, donnees.ehdr, donnees.shdrTable, intSec, donnees.strSec); //Pour l'instant ça prend en compte que le numéro qu'on met ici
        }
  else {
  ...
  if (option4) {
		print_symbole(fd, donnees.etape4Names, donnees.symTable, donnees.nbsymbole);
  }

  if (option5) {
        print_reimp(fd, donnees.ehdr, donnees.shdrTable, donnees.tabNames, donnees.rel, error);
  }
  ...
  if (option6 != NULL) {
				 ...
				 if (argc >= 3) {
					 ...
					 //ETAPE LECTURE
				 	 ...
				 	 //ETAPE FUSION 
					 fd3 = open(option6, O_CREAT | O_WRONLY); //Je crée un nouveau fichier là, pis je vérifie juste si c'est bien crée.
					if (fd3 < 0) {
						printf("Erreur open : %s\n", option6);
						exit(0);
					}
					 
					fusion_section_write(fd3,donnees,donnees2);
