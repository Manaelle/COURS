...
int print_reimp(int fd, Elf32_Ehdr *ehdr, Elf32_Shdr *shdrTable, char *tabNames, Elf32_Rel *rel, int *error){
    ...
    int i=0, nbentree=0, j=0 ;

		//LECTURE
		while(i<ehdr->e_shnum){
     //se déplacer au début de la section .rel.text
     
    if(shdrTable[i].sh_type == SHT_REL){ //NE PAS FAIRE LES RELA
        lseek(fd, shdrTable[i].sh_offset, SEEK_SET);
        nbentree = shdrTable[i].sh_size / sizeof(Elf32_Rel); //pour afficher le nombre d'entree

        rel = malloc(sizeof(Elf32_Rel) * nbentree);

        j=0;
        
        while(j<nbentree){
            read(fd, &rel[j], sizeof(rel));
            j++;
        }
				
				//AFFICHAGE
				printf("*************************************************\n");
        printf("Section de relocalisation '%s' a l'adresse de decalage 0x%x contient %d entrees: \n \n", tabNames + shdrTable[i].sh_name, shdrTable[i].sh_offset, nbentree);
        ...
        while(j<nbentree){
            ...
            printf("%8.8x \t", rel[j].r_offset); //Decalage
            printf("%8.8x \t", rel[j].r_info); //Info
            ...
            printf("%8.8x \t", ((ELF32_R_SYM(rel[j].r_info))>>8)); //Val.-sym
